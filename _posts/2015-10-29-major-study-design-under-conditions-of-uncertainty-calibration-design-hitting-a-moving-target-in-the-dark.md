...
<figcaption>
 PredictionBook screen shots
</figcaption>
</figure>

<img src="{{ site.baseurl }}/assets/dip/image108.png" alt="image108" />Using checklists doesn’t do anything to address to root of over-confidence, and empirical observations are a very slow way of discovering the accuracy of one’s predictions. Betting on stock markets or horses is one way to test your calibration very quickly, but it requires a certain level of expert knowledge, and also discourages exploration of low certainty events. Explicit calibration training methods allow measuring of prediction specific to one’s field of knowledge.

<a href="http://predictionbook.com/">Prediction book</a> is a website that allows people to make predictions and state a confidence percentage in that prediction. This allows an idea of one’s calibration over time to be built up with reference to a range of real events. I.e. I may be very well calibrated about my ability to complete a particular grade of climb, but very poorly calibrated about the length of time to complete a list of features in a spread sheet. If Prediction book or something similar were to be used frequently then the user would quickly start to see feedback related to their confidence.

<figure class="full-width">
<img class="" src="{{ site.baseurl }}/assets/dip/image111.jpg" alt="image111" />
<figcaption>
 Wit and Wagers Board
</figcaption>
</figure>
...
<figcaption>Edward Nygma - aka The Riddler 1992<a href="#_ftn23" name="_ftnref23">[23]</a>
</figcaption>
</figure>

<img class="size-full alignnone" src="{{ site.baseurl }}/assets/dip/image112.png" alt="image112" />
...
The simplest decision support system is asking for advice from a friend. That advice still requires some sort of assessment of whether or not to include it in your final decision. The ultimate responsibility lies with the human interpreting the results. The same is true of a computational DSS, it merely provides a recommendation based on some inputs, the final synthesis of that advice is down to the designer.

<figure class="half-width left">
<img src="{{ site.baseurl }}/assets/dip/image113.png" alt="image113" />
<figcaption>
 Some examples of the spelling and grammar checking in Microsoft Word.
</figcaption>
</figure>

A commonly encountered example of a decision support system is the coloured underlining in Microsoft word. If a word is thought to be incorrectly spelt, it is underlined in red, if it is thought to be grammatically incorrect then it is underlined in green, and if the word is correctly spelt, but it is an incongruous use of that word then it will be underlined in blue. If the user right clicks on the underlined word options to ignore or accept a suggested correction are given. The decision still lies with the user, but they are supported in making it by the computer.

An initial look at the literature makes the field look extremely impenetrable, but this is simply an illusion caused by the range of subjects covered being extremely broad. Any system that can support a human decision maker would count as a DSS, so this can vary from visual dashboard design for real-time data feedback to a system for modelling the behaviour of people in a train station.
...
The specific causes of over-confidence are still unclear, but they can often be attributed to reliance on an out of date heuristics. The Carbon Buzz data implies that our mental picture of energy use is based on the most prevalent model, in this case the part L calculations. These calculations are not ‘wrong’, but they provide a picture that is a little more reassuring that it ought to be because they don’t remind us that they model only a very small part of the real scenario.

As information is a volatile commodity we ought to be careful while using it. The example of the proportion of the UK housing stock that will be extant in 2050 illustrates this difficulty well as the paper that superseded the _40% house_ information from 2005 was published in 2008. The difficulty stems from published material not feeding a central repository of knowledge; therefore there is no way to tell if information is outdated. Good calibration would allow a judge to decide if they ought to spend time searching for newer data. It would be useful if some form of decision support system was to be implemented that could answer these sorts of questions, but the challenge of creating a system that can respond to human language questions is nontrivial.

Checklists avoid some of the need to be well calibrated. The qualms about them leading to a mentality that designs to fit the criteria set out by the boxes are valid, but can be avoided by maintaining a ‘bigger picture’ view of the design process. The problems that checklists have run into before have often been because the checklist was the measure of success, and not the quality of the outcome.

As was shown in the example of designing a mouse, there is a need to be cautious of our intuition about the performance of our solutions. The designers at Apple got around this by user testing; creating verified knowledge through experimentation and prototyping. There is no good reason that architecture shouldn’t do the same. The current model used to predict building emissions has been shown by Carbon Buzz to be inadequate, but this doesn’t necessarily invalidate the model, just its use. If it is used by experienced engineers to predict the energy use they will compensate for its shortcomings, but by how much remains a dark art until there is good data available to calibrate against.

In our relationships with consultants being well calibrated can lead to engaging with people who have the expert skills that we lack early on before our own ignorance of that specific area becomes a problem. By getting their creative input, rather than just relying on the to bail us out of problems, a much richer product can result.

The most important thing that this study shows it that people are most over-confident about things that they are most confident about; therefore the more sure you are about something, the more suspicious you ought to be of it. Just having an awareness of calibration of a concept seems to be enough to have some effect on one’s overconfidence.
...
<span class="author">Alexander, C.</span> <span class="date">(1964)</span>. <cite>Notes on the Synthesis of Form</cite>. Cambridge, Massachusetts: Harvard University Press. Retrieved from <a href="http://www.amazon.co.uk/Notes-Synthesis-Form-Harvard-Paperbacks/dp/0674627512.">http://www.amazon.co.uk/Notes-Synthesis-Form-Harvard-Paperbacks/dp/0674627512.</a>

<span class="author">Allingham, M.</span> <span class="date">(2002)</span>. <cite>Choice Theory: A Very Short Introduction (p</cite>. 144). <cite>Oxford: Oxford University Press</cite>. Retrieved from <a href="http://www.oup.com/us/catalog/general/subject/Economics/Theory/?view=usa&amp;ci=9780192803030.">http://www.oup.com/us/catalog/general/subject/Economics/Theory/?view=usa&amp;ci=9780192803030.</a>

<span class="author">Ambrose, G., &amp; Harris, P.</span> <span class="date">(2009)</span>. <cite>Basics Design: Design Thinking: 8 (1 ed</cite>., p. 184). <cite>Lausanne, Switzerland: Ava Publishing</cite>. Retrieved from <a href="http://www.amazon.co.uk/Basics-Design-Thinking-8/dp/2940411174.">http://www.amazon.co.uk/Basics-Design-Thinking-8/dp/2940411174.</a>

<span class="author">Ayres, I.</span> <span class="date">(2008)</span>. <cite>Supercrunchers</cite>. Bantam Trade. Retrieved from <a href="http://www.randomhouse.com/bantamdell/supercrunchers/.">http://www.randomhouse.com/bantamdell/supercrunchers/.</a>

<span class="author">Barber, B. M., &amp; Odean, T.</span> <span class="date">(2000)</span>. <cite>Trading Is Hazardous to Your Wealth: The Common Stock Investment Performance of Individual Investors</cite>. The Journal of Finance, Vol 55(Issue 2), 773-806. Retrieved from <a href="http://www.jstor.org/stable/222522.">http://www.jstor.org/stable/222522.</a>

<span class="author">Bellinger, G., Castro, D., &amp; Mills, A.</span> <span class="date">(2004)</span>. <cite>Data, Information, Knowledge, and Wisdom</cite>. systems-thinking.org. Retrieved from <a href="http://www.systems-thinking.org/dikw/dikw.htm.">http://www.systems-thinking.org/dikw/dikw.htm.</a>

<span class="author">Boardman, B., Darby, S., Killip, G., Hinnells, M., Jardine, C. N., Palmer, J., et al.</span> <span class="date">(2005)</span>. <cite>40% House</cite>. Environmental Change Institute, University of Oxford, 5. Environmental Change Institute, University of Oxford. Retrieved from <a href="http://www.eci.ox.ac.uk/research/energy/downloads/40house/40house.pdf.">http://www.eci.ox.ac.uk/research/energy/downloads/40house/40house.pdf.</a>

<span class="author">Brier, G.</span> <span class="date">(1950)</span>. <cite>Verification of forecasts expressed in terms of probability</cite>. Monthly weather review, 78(1), 1–3. American Meteorological Society. Retrieved from <a href="http://ams.allenpress.com/perlserv/?request=get-abstract&amp;doi=10.1175/1520-0493(1950)0782.0.CO;2.">http://ams.allenpress.com/perlserv/?request=get-abstract&amp;doi=10.1175/1520-0493(1950)0782.0.CO;2.</a>

<span class="author">Brown, T. A., &amp; Shuford, E. H.</span> <span class="date">(1973)</span>. <cite>Quantifying uncertainty into numerical probabilities for the reporting of intelligence</cite>. Report R-1185-ARPA. Santa Monica, CA.

<span class="author">Christensen-Szalanski, J., &amp; Bushyhead, J.</span> <span class="date">(1981)</span>. <cite>Physicians's use of probabilistic information in a real clinical setting</cite>. Journal of Experimental Psychology: Human Perception and Performance, 7.

<span class="author">Cooke, E.</span> <span class="date">(1906)</span>. <cite>Forecasts and Verifications in Western Australia</cite>. Monthly Weather Review, 34(1). <cite>doi: 10</cite>.1175/1520-0493(1906)34&lt;23:FAVIWA&gt;2.0.CO;2.
...
<span class="author">DECC.</span> <span class="date">(2010)</span>. <cite>CRC user guide and guidance documents</cite>. Retrieved from <a href="http://www.decc.gov.uk/en/content/cms/what_we_do/lc_uk/crc/user_guidance/user_guidance.aspx.">http://www.decc.gov.uk/en/content/cms/what_we_do/lc_uk/crc/user_guidance/user_guidance.aspx.</a>

<span class="author">Dawkins, R.</span> <span class="date">(1988)</span>. <cite>Evolution of evolvability</cite>. In C. Langton, Artificial Life, SFI Studies in the Sciences of Complexity (pp. 201-220). <cite>Addison-Wesley Publishing Company</cite>. Retrieved from <a href="http://www3.interscience.wiley.com/journal/119081874/abstract.">http://www3.interscience.wiley.com/journal/119081874/abstract.</a>

<span class="author">Delaney, J.</span> <span class="date">(2003)</span>. <cite>intrade</cite>.com. Retrieved from <a href="http://www.intrade.com/.">http://www.intrade.com/.</a>

<span class="author">Doherty, B.</span> <span class="date">(2009)</span>. <cite>Genetic Algorythms and Pirates</cite>. Delft. Retrieved from <a href="http://c-p.bk.tudelft.nl/events/Open Platform.html.">http://c-p.bk.tudelft.nl/events/Open Platform.html.</a>

<span class="author">Doxiadis, A., &amp; Papadimitriou, C. H.</span> <span class="date">(2009)</span>. <cite>Logicomix: An Epic Search for Truth</cite>. London: Bloomsbury Publishing PLC. Retrieved from <a href="http://www.logicomix.com/en/.">http://www.logicomix.com/en/.</a>

<span class="author">Feynman, R.</span> <span class="date">(1981)</span>. <cite>The Pleasure of Finding Things Out</cite>. Horizon.

<span class="author">Gawande, A.</span> <span class="date">(2010)</span>. <cite>The Checklist Manifesto: How to get things right</cite>. London: Profile Books. Retrieved from <a href="http://www.amazon.co.uk/Checklist-Manifesto-How-things-right/dp/1846683130/ref=sr_1_1?ie=UTF8&amp;s=books&amp;qid=1271579585&amp;sr=1-1#.">http://www.amazon.co.uk/Checklist-Manifesto-How-things-right/dp/1846683130/ref=sr_1_1?ie=UTF8&amp;s=books&amp;qid=1271579585&amp;sr=1-1#.</a>

<span class="author">Gilovich, T., Griffin, D. W., &amp; Kahneman, D.</span> <span class="date">(2003)</span>. <cite>Heuristics and biases: the psychology of intuitive judgement (p</cite>. 59). <cite>Cambridge, United Kingdom: Cambridge University Press</cite>. Retrieved from <a href="http://books.google.com/books?hl=en&amp;lr=&amp;id=FfTVDY-zrCoC&amp;pgis=1.">http://books.google.com/books?hl=en&amp;lr=&amp;id=FfTVDY-zrCoC&amp;pgis=1.</a>

<span class="author">Hanson, R.</span> <span class="date">(1989)</span>. <cite>Encouraging an Honest Consensus</cite>. Extropy, 1-11.

<span class="author">Hanson, R.</span> <span class="date">(2008)</span>. <cite>Catastrophe, social collapse, and and human extinction</cite>. In N. Bostrom &amp; M. Cirkov, Global Catastrophic Risks. Oxford: OUP.

<span class="author">Hillier, B., Leaman, A., &amp; Stansall, P.</span> <span class="date">(1976)</span>. <cite>Space syntax</cite>. Environment and Planning B, 3, 147-185. Retrieved from <a href="http://www.environmentandplanning.com/epb/fulltext/b03/b030147.pdf.">http://www.environmentandplanning.com/epb/fulltext/b03/b030147.pdf.</a>

<span class="author">IPCC.</span> <span class="date">(2003)</span>. <cite>IPCC report</cite>.

<span class="author">IPCC.</span> <span class="date">(2007)</span>. <cite>IPCC report</cite>.

<span class="author">Kahneman, D.</span> <span class="date">(2009)</span>. <cite>Nobelist Daniel Kahneman on Behavioral Economics</cite>. fora.tv. Retrieved from <a href="http://fora.tv/2009/05/15/Nobelist_Daniel_Kahneman_on_Behavioral_Economics.">http://fora.tv/2009/05/15/Nobelist_Daniel_Kahneman_on_Behavioral_Economics.</a>

<span class="author">Keen, P. G., &amp; Morton, M. S.</span> <span class="date">(1978)</span>. <cite>Decision Support Systems: An Organizational Perspective</cite>. Addison Wesley.

<span class="author">Kilian, A.</span> <span class="date">(2006)</span>. <cite>Design Exploration through Bidirectional Modeling of Constraints</cite>. Design.

<span class="author">Kimpian, J., &amp; Doherty, B.</span> <span class="date">(2010)</span>. <cite>Judit Kimpian On Carbon Buzz</cite>. Notion Parallax. Retrieved from <a href="http://www.notionparallax.co.uk/blog/index.php/2010/04/judit-kimpian-on-carbon-buzz/#more-460.">http://www.notionparallax.co.uk/blog/index.php/2010/04/judit-kimpian-on-carbon-buzz/#more-460.</a>

<span class="author">Kimpian, J., Davies, E., Bassi, R., Muthoni, S., Davies, H., Woods, P., et al.</span> <span class="date">(2008)</span>. <cite>Carbon Buzz Handbook (p</cite>. 38). <cite>London</cite>. Retrieved from www.carbonbuzz.org.

<span class="author">Kuhn, T. S.</span> <span class="date">(1962)</span>. <cite>The Structure of Scientific Revolutions</cite>. Chicago University Press. Retrieved from <a href="http://www.amazon.co.uk/Structure-Scientific-Revolutions-TS-Kuhn/dp/0226458083.">http://www.amazon.co.uk/Structure-Scientific-Revolutions-TS-Kuhn/dp/0226458083.</a>

<span class="author">Kurzweil, R.</span> <span class="date">(2001)</span>. <cite>The law of accelerating returns</cite>. KurzweilAI. Retrieved from KurzweilAI.net.

<span class="author">Lichtenstein, S., &amp; Fischhoff, B.</span> <span class="date">(1977)</span>. <cite>Do those who know more also know more about how much they know? The calibration of probability judgments</cite>. Organizational Behavior and Human Performance, 20.

<span class="author">Lichtenstein, S., Fischhoff, B., &amp; Phillips, L. D.</span> <span class="date">(1981)</span>. <cite>Calibration of Probabilities: The State of the Art to 1980</cite>. Judgment under uncertainty: heuristics and biases, 306-354. Retrieved from <a href="http://oai.dtic.mil/oai/oai?verb=getRecord&amp;metadataPrefix=html&amp;identifier=ADA101986.">http://oai.dtic.mil/oai/oai?verb=getRecord&amp;metadataPrefix=html&amp;identifier=ADA101986.</a>

<span class="author">MacKay, D.</span> <span class="date">(2009)</span>. <cite>Sustainable Energy without the hot air</cite>.

<span class="author">McCabe, T.</span> <span class="date">(2009)</span>. <cite>Test Your Calibration! Less Wrong</cite>. Retrieved from <a href="http://lesswrong.com/lw/1f8/test_your_calibration/.">http://lesswrong.com/lw/1f8/test_your_calibration/.</a>

<span class="author">Moggridge, B.</span> <span class="date">(2006)</span>. <cite>Designing Interactions (p</cite>. 766). <cite>MIT Press</cite>. Retrieved from <a href="http://www.amazon.co.uk/Designing-Interactions-B-Moggridge/dp/0262134748/ref=sr_1_1?ie=UTF8&amp;s=books&amp;qid=1271579334&amp;sr=8-1.">http://www.amazon.co.uk/Designing-Interactions-B-Moggridge/dp/0262134748/ref=sr_1_1?ie=UTF8&amp;s=books&amp;qid=1271579334&amp;sr=8-1.</a>

<span class="author">Murphy, A.</span> <span class="date">(1973)</span>. <cite>A new vector partition of the probability score</cite>. Journal of Applied Meteorology, 12, 595-600. Retrieved from <a href="http://ams.allenpress.com/amsonline/?request=get-abstract&amp;issn=1520-0450&amp;volume=012&amp;issue=04&amp;page=0595.">http://ams.allenpress.com/amsonline/?request=get-abstract&amp;issn=1520-0450&amp;volume=012&amp;issue=04&amp;page=0595.</a>

<span class="author">Oppenheimer, M., O’Neill, B. C., &amp; Webster, M.</span> <span class="date">(2008)</span>. <cite>Negative learning</cite>. Climatic Change, 89(1-2), 155-172. doi: 10.1007/s10584-008-9405-1.

<span class="author">Paran, G.</span> <span class="date">(2008)</span>. <cite>The DeFinetti game</cite>. abstract confusions. Retrieved from <a href="http://guruparan18.wordpress.com/2008/05/10/the-de-finetti-game/.">http://guruparan18.wordpress.com/2008/05/10/the-de-finetti-game/.</a>

<span class="author">Price, C.</span> <span class="date">(1990)</span>. <cite>Talks at the AA</cite>. AA Files, 19(Spring), p. 32. AA Publications. Retrieved from <a href="http://www.aaschool.ac.uk/publications/Main.aspx?sectionId=1&amp;entryId=283.">http://www.aaschool.ac.uk/publications/Main.aspx?sectionId=1&amp;entryId=283.</a>

<span class="author">Roberts, R., &amp; Ayres, I.</span> <span class="date">(1999)</span>. <cite>Ayres on Data</cite>. Econtalk.

<span class="author">Roberts, R., &amp; Hanson, R.</span> <span class="date">(2009)</span>. <cite>Hanson on health</cite>. EconTalk. Retrieved from <a href="http://www.econtalk.org.">http://www.econtalk.org.</a>

<span class="author">Roberts, R., &amp; Sunstein, C. R.</span> <span class="date">(2007)</span>. <cite>Sunstein on Infotopia, Information and Decision-Making</cite>. Fairfax, Virginia: Econtalk. Retrieved from <a href="http://www.econtalk.org/archives/2007/05/sunstein_on_inf.html.">http://www.econtalk.org/archives/2007/05/sunstein_on_inf.html.</a>

<span class="author">Rumsfeld, D. H.</span> <span class="date">(2002)</span>. <cite>DoD News Briefing - Secretary Rumsfeld and Gen</cite>. Myers. U.S. Department of Defense. Retrieved from <a href="http://www.defense.gov/transcripts/transcript.aspx?transcriptid=2636.">http://www.defense.gov/transcripts/transcript.aspx?transcriptid=2636.</a>

<span class="author">Salvatier, J., &amp; Loddengaard, A.</span> <span class="date">(2008)</span>. <cite>Calibrated Probability Assessment</cite>. Retrieved from <a href="http://calibratedprobabilityassessment.org.">http://calibratedprobabilityassessment.org.</a>

<span class="author">Statman, M., Thorley, S., &amp; Vorkink, K.</span> <span class="date">(2006)</span>. <cite>Investor Overconfidence and Trading Volume</cite>. Rev. Financ. Stud., 19(4), 1531-1565. doi: 10.1093/rfs/hhj032.

<span class="author">Sunstein, C. R.</span> <span class="date">(2006)</span>. <cite>Infotopia: How Many Minds Produce Knowledge</cite>. Oxford University Press, USA. Retrieved from <a href="http://www.amazon.com/Infotopia-Many-Minds-Produce-Knowledge/dp/0195189280.">http://www.amazon.com/Infotopia-Many-Minds-Produce-Knowledge/dp/0195189280.</a>

<span class="author">Sutherland, S.</span> <span class="date">(1992)</span>. <cite>Irrationality</cite>. Pinter &amp; Martin Ltd. Retrieved from <a href="http://www.amazon.co.uk/Irrationality-Stuart-Sutherland/dp/1905177070.">http://www.amazon.co.uk/Irrationality-Stuart-Sutherland/dp/1905177070.</a>

<span class="author">Svenson, O.</span> <span class="date">(1981)</span>. <cite>Are we all less risky and more skillful than our fellow drivers? Acta Psychologica, 47(2), 143-148</cite>. doi: DOI: 10.1016/0001-6918(81)90005-6.

<span class="author">Taleb, N. N.</span> <span class="date">(2008)</span>. <cite>The Black Swan: The Impact of the Highly Improbable</cite>. Penguin. Retrieved from <a href="http://www.amazon.co.uk/Black-Swan-Impact-Highly-Improbable/dp/0141034599.">http://www.amazon.co.uk/Black-Swan-Impact-Highly-Improbable/dp/0141034599.</a>
...
<a href="#_ftnref2" name="_ftn2">[2]</a> There are plenty of scholarly texts on this, but the graphic novel Logicomix (Doxiadis &amp; Papadimitriou, 2009) explains the history of the search for ‘truth’ far better than I ever could.
...
<a href="#_ftnref6" name="_ftn6">[6]</a> Thomas Kuhn mentions that the greatest leaps of creativity occur in science when a field appears to be totally constrained. A Good example of this is Euclidean geometry. Assuming the constraints of planar geometry, Euclid’s fifth postulate, the parallel postulate, remains true. Once a thinking about parallel lines in non planar space the postulate falls. (lines of longitude are parallel at the equator, and cross at the poles)

<a href="#_ftnref7" name="_ftn7">[7]</a> The main UK political parties all appointed dedicated climate change ministers in 2009, and before this the role had been rolled into the energy minister’s portfolio since at least the 90s.

<a href="#_ftnref8" name="_ftn8">[8]</a> It is very difficult to find exact data for this as there is no official designation of the boundary between sectors. It seems that if the report wishes to make it look like a particular sector emits more, it just includes more things in that sector, and vice versa.

<a href="#_ftnref9" name="_ftn9">[9]</a> This is a view that I believe is shared by David MacKay the author of _Sustainable Energy Without the Hot Air_. The introduction _Numbers, not adjectives_ is a beautifully written plea for people to learn basic numerical and analytical skills to allow them to make informed decisions.
...
Table 2.14, which lists the global warming potentials for various greenhouse gasses, can be found here: <a href="http://tinyurl.com/gwpTable">http://tinyurl.com/gwpTable</a> and the above text can be found here: <a href="http://tinyurl.com/indirectGWPs">http://tinyurl.com/indirectGWPs</a>.

<a href="#_ftnref12" name="_ftn12">[12]</a> Most words in italics are defined in the terminology section of the appendix. There are a few cases where italics are used for emphasis, but these should be fairly self evident.

<a href="#_ftnref13" name="_ftn13">[13]</a> The current audience is architects, in future work the range of professions surveyed will be expanded. Currently there are a number of other fields being tested, engineers, policy makers, and product designers. This was driven as much by the people who i was able to survey as by a deliberate choice of other fields to provide a comparison.

<a href="#_ftnref14" name="_ftn14">[14]</a> The site is implemented in PHP, the questions are marked by comparing the answer given with the answer that is stored as the correct one. This is done using a simple string comparison. This crude method was surprisingly useful in that it allows minor changes to be made to the questions after launch, for example making the 2 in CO<sub>2</sub> subscript without needing to do any recoding of the marking scheme.

<a href="#_ftnref15" name="_ftn15">[15]</a> I’ve seen Oxford listed and South East, Midlands, and South West.

<a href="#_ftnref16" name="_ftn16">[16]</a> PHP is explained in the terminology section of the appendix, but in brief, it is a programming language used to make websites _do_ things.

<a href="#_ftnref17" name="_ftn17">[17]</a> AJAX is a conglomeration of a few web technologies that make it relatively easy to have dynamic sections of web content that can do things like checking that an email address is well formed, or fetching some content without needing to refresh the whole page. It is short for asynchronous JavaScript and XML.

<a href="#_ftnref18" name="_ftn18">[18]</a> Initially usefulness was written as utility, but in the context of a discussion of rationality, utilitarianism is often close behind, and in the interest of clarity, the less elegant word was chosen.
...
Dawkins shows this in _The Blind watchmaker_ with the now famous weasel thought experiment. He tries to evolve the phrase “methinks it is like a weasel” from a random string of characters. It takes him 11 seconds to achieve this, and using the same computer, but with single-step selection it would take: “about a million million million million million years. This is more than a million million million times as long as the universe has so far existed”

<a href="#_ftnref20" name="_ftn20">[20]</a> For some reason some of the results came through a little mangled. Some were salvageable, but one or two were garbled.

<a href="#_ftnref21" name="_ftn21">[21]</a> If a subject refreshed the page while they were looking at the results then the results would be submitted again. This can happen with modern browsers if the session is stored and the browser closed as it will refresh when it opens.

<a href="#_ftnref22" name="_ftn22">[22]</a> Better performing buildings often cost more than their lower performance counterparts, but this isn’t a rule. Through excellent design a building can be made to actually be cheaper, especially when operating costs are taken into account.

<a href="#_ftnref23" name="_ftn23">[23]</a> This quote can also be attributed to an argument between an environmentalist and an economist. The environmentalist made some predictions about how resource prices in the future would rise as a result of some sort of ecological disaster. The economist responded with “If you are so smart, why aren’t you rich?” The argument was settled with a bet, if the resource prices did rise in line with the environmentalist’s predictions the economist would pay out a nominal sum (a few thousand dollars) and vice versa. Unfortunately the source of this parable completely escapes me, so this quote is taken from a Batman cartoon.

<a href="#_ftnref24" name="_ftn24">[24]</a> Crystal Sarah Bowersox (born August 4, 1985) is an American singer-songwriter from Elliston, Ohio. As far as I can tell her second name is actually real!

<a href="#_ftnref25" name="_ftn25">[25]</a> “On Dec. 11, 2003, Intrade's contract on Saddam Hussein's capture suddenly began to move. Two days later, Saddam was in custody.” Bill Saporito, Time Magazine
